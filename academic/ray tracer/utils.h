...
//Helper function I wrote to multiply a vector by a scalar.
inline void scalarVecMult(double s, struct point3D *pt)
{
  struct point3D pr;
  pr.px=(s*pt->px);
  pr.py=(s*pt->py);
...
void boxIntersect(struct object3D *box, struct ray3D *r, double *lambda, struct point3D *p, struct point3D *n, double *a, double *b);
...
void addPlanarLight(double sx, double sy, double rx, double ry, double rz,\
                  double tx, double ty, double tz,\
                  double r, double g, double b, struct object3D **o_list, struct pointLS **l_list, struct textureNode **t_list, const char *filename);
